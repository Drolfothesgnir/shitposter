...
func TestDownvoteCommentTx(t *testing.T) {
	n := 10
	comment1 := createRandomComment(t)

	errs := make(chan error)
	results := make(chan DownvoteCommentTxResult)

	for range n {
		go func() {
			user := createRandomUser(t)

			result, err := testStore.DownvoteCommentTx(context.Background(), DownvoteCommentTxParams{
				UserID:    user.ID,
	...
	require.Equal(t, int64(n), comment2.Downvotes)
}
