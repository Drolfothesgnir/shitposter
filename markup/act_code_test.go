...
func TestActCode_ClosedBlock_Simple(t *testing.T) {
	substr := "```a```"
	warns := make([]Warning, 0)
...
func TestActCode_OnlyBackticks_ReturnedAsText(t *testing.T) {
	...
	require.Len(t, warns, 1)
	require.Equal(t, IssueMalformedCodeSequence, warns[0].Issue)
	require.Equal(t, 0, warns[0].Index)
}
...
func TestActCode_Block_AllowsLongerBacktickRunsInsideContent(t *testing.T) {
	...
	tok, stride := actCode(input, 0, &warns)
	t.Log(tok)
	require.Empty(t, warns)

	require.Equal(t, TypeCodeBlock, tok.Type)
	require.Equal(t, 0, tok.Pos)
	require.Equal(t, len(input), tok.Len)
	require.Equal(t, input, tok.Val)

	require.Equal(t, len(input), stride)
}
