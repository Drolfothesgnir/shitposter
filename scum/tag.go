...
type Tag struct {
	...
	// Greed is a measure of how many next characters the Tag must consume until the closing Tag occurs during tokenization.
	...
	// Token's internal value. But if the Tag is unclosed, only the opening Tag is consumed, just like with Greed 0,
	...
	//
	// 2 - the Tag-VS-Content rule. Can be used by only a greedy Tag. The rule is used to avoid confusion of plain text and the closing tag
	// during the tokenization process. The rule allows a universal single-byte Tag to have variable-width opening and closing sequences.
	...
	//    the "CODE" will capture "const T = a && b;" as its content, even though the Tag is single and there is "&&" in the content.
	Rule Rule
	...
	// WARNING: You have to set at least one of OpenID or CloseID for the Parser to consider the Tag valid.
	OpenID byte
...
// ID returns the unique byte value used as the opening byte for the Tag's sequence.
func (t *Tag) ID() byte {
	return t.Seq.ID()
}

// IsUniversal returns true when the Tag's signature is the same as its closing [Tag].
func (t *Tag) IsUniversal() bool {
	return t.CloseID == t.ID() && t.OpenID == t.ID()
}

// IsOpening returns true when the Tag opens a non-universal tag pair.
func (t *Tag) IsOpening() bool {
	return t.CloseID != 0 && t.OpenID == 0
}

// IsClosing returns true when the Tag closes a non-universal tag pair.
func (t *Tag) IsClosing() bool {
	return t.OpenID != 0 && t.CloseID == 0
}

// Len returns the length in bytes of the Tag's sequence.
...
// WithGreed sets the [Greed] level for a [Tag] created by [NewTag].
...
// WithRule sets the [Rule] for a [Tag] created by [NewTag].
